...
import { LocalDevicesPanel } from "./components/domain/local-devices-panel.js";
...
export function ReaderSurface({ principal, route, request, requestBinary, onLogout, onNavigate, canUseAdministration }: ReaderSurfaceProps) {
  ...
  return <div className={`app-shell reader-shell ${isNavCollapsed ? "nav-collapsed" : ""} ${accountSettings ? "reader-shell--account" : ""}`} style={shellStyle}>
    ...
    <main className={`reader-main ${accountSettings ? "reader-main--account" : ""}`} id="main">
      {accountSettings ? <section className="account-settings-page" aria-labelledby="account-settings-title">
        <header className="account-settings-header"><p className="eyebrow">Account</p><h1 id="account-settings-title">Settings</h1><p>Review the signed-in identity, role, groups, and access scopes used for this session.</p></header>
        <dl className="account-settings-grid"><div><dt>Name</dt><dd>{displayIdentity}</dd></div><div><dt>Email</dt><dd>{principal.email ?? "not available"}</dd></div><div><dt>Role</dt><dd>{principal.role}</dd></div><div><dt>Principal</dt><dd>{principal.principalType}</dd></div><div><dt>Groups</dt><dd>{formatList(principal.groupIds)}</dd></div><div><dt>Scopes</dt><dd>{formatList(principal.scopes)}</dd></div></dl>
        <LocalDevicesPanel request={request} />
        <div className="account-settings-actions">{canUseAdministration ? <Button type="button" onClick={() => onNavigate("admin/content")}>Admin</Button> : null}<Button type="button" variant="ghost" onClick={() => void onLogout()}>Sign out</Button></div>
      </section> : <>
