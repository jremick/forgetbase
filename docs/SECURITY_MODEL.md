...
Browser login also sets an `agentic_cms_csrf` cookie. The operational web UI echoes that value in `x-agentic-cms-csrf` for unsafe cookie-authenticated requests. Operators do not need to configure this token, but custom browser clients must implement the same header echo or use bearer authorization.
