...
The release proof bundle is a JSON manifest plus referenced screenshots/log summaries. It must pass `release-proof:check`; the check rejects placeholder values, missing screenshots, localhost live-demo URLs, missing required gates, missing compatibility evidence, and missing GitHub/security read-back evidence.
