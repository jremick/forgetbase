...
This document records what is not actually complete after the alpha functional closeout. It should prevent the project from claiming production, hosted-service, or full orchestration readiness before the evidence exists.
...
- Permission-filtered retrieval through lexical, deterministic hash-vector, and hybrid strategies.
- REST/OpenAPI, SDK, CLI, MCP, worker, operational web UI, AI export, and OKF v0.1 export projection.
...
- Provider-quality semantic embeddings, embedding model selection, reindex jobs, semantic reranking, per-tenant retrieval profiles, and search-service federation.
...
- Generated OpenAPI or a stronger route/schema drift check. The current contract is hand-authored with targeted tests.
- CLI contract tests against a disposable API fixture.
- MCP server contract tests for tool listing, search/fetch, validate-context-access, managed query, and export.
- Compose smoke automation that boots the stack, bootstraps an admin, imports corpus, searches, and checks same-origin proxy health.
- CI or release automation for `security:verify-restricted-leakage` and `db:verify-backup-restore`.
- Browser/UAT automation after the separate UI design review lands.

## Current Boundary

The alpha can be described as a private/public-alpha candidate for the self-hosted open-source core. It should not be described as production-ready, hosted-service-ready, stable-API-compatible, enterprise-identity-complete, or full managed-agent orchestration.
