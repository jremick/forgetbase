...
ForgetBase should be implemented as a TypeScript monorepo with containerized services, Postgres as the system of record, shared domain/schema packages, and first-class API, CLI, MCP, worker, and web surfaces.
...
Current implementation note: the worker reindexes retrieval chunks on `--once`, can run telemetry retention maintenance through `--retention-once`, can run expired managed-query cache cleanup through `--cache-purge-once`, can run service-account API-key rotation reminder maintenance through `--api-key-rotation-reminders-once`, can run due deterministic managed-query eval schedules through `--managed-query-evals-once`, and can run expired action approval maintenance through `--action-approval-expiry-once`. Long-running workers can schedule retention maintenance with `FORGETBASE_RETENTION_PURGE_ENABLED=true`, cache cleanup with `FORGETBASE_CACHE_PURGE_ENABLED=true`, API-key rotation reminders with `FORGETBASE_API_KEY_ROTATION_REMINDERS_ENABLED=true`, deterministic eval schedules with `FORGETBASE_MANAGED_QUERY_EVALS_ENABLED=true`, and action approval expiry with `FORGETBASE_ACTION_APPROVAL_EXPIRY_ENABLED=true`; all maintenance jobs remain dry-run unless the matching dry-run env var is set to `false`. Reminder execution dedupes matching tenant/key-state audit evidence for 24 hours by default; set `FORGETBASE_API_KEY_ROTATION_REMINDERS_DEDUPE_WINDOW_HOURS=0` only when duplicate audit events are intentionally needed. Executed, non-duplicate reminder reports can optionally POST a reduced tenant reminder payload to `FORGETBASE_API_KEY_ROTATION_REMINDERS_WEBHOOK_URL`; dry-runs never deliver, payloads omit raw secrets and secret previews, and `FORGETBASE_API_KEY_ROTATION_REMINDERS_WEBHOOK_SIGNING_SECRET` adds an HMAC SHA-256 signature header without storing the signing secret. Executed action approval expiry maintenance marks stale pending approvals `expired`, records `agent.action.approval_expiry` audit evidence, and does not execute actions.
...
Current implementation note: page attachments use tenant/asset-scoped metadata in Postgres and opaque storage keys behind a bounded adapter. The local filesystem adapter rejects unsafe keys, traversal, symlinks, duplicate writes, and reads above the configured byte limit; writes publish through an fsynced temporary file and fail rather than replace an existing blob. Uploads must pass the allowlisted extension, declared media type, content signature/structure check, uploader and tenant quota checks, IP rate limit, concurrency gate, and malware scan when scanning is required. Compose runs a pinned ClamAV daemon on the internal network and fails readiness and uploads closed when it is unavailable. The default 10 MiB upload cap and four-request concurrency gate bound buffered request memory for the self-hosted prototype; this is not a streaming upload implementation. Downloads authorize the parent asset first, require a clean scan record when scanning is required, verify metadata-to-blob integrity, and force `Content-Disposition: attachment` with `nosniff` and `no-store`. Deletion first marks metadata `deleting`, removes the blob immediately, then tombstones metadata as `deleted`; a storage failure leaves a non-readable retryable state. Audit failure after committed metadata does not delete the referenced blob. Deleted metadata is retained for audit/recovery context until a future operator-governed purge policy is defined. Compose uses a dedicated attachment volume. Backup and restore capture Postgres and that volume as one stopped-writer, checksummed backup set. The S3-compatible adapter, multi-instance transactional quota reservations, and content-disarm/reconstruction remain future work.

Attachment API routes are `GET|POST /assets/{stableId}/attachments`, `GET /assets/{stableId}/attachments/{attachmentId}/download`, `DELETE /assets/{stableId}/attachments/{attachmentId}`, and admin-only `POST /admin/attachments/reconcile`. Admin reconciliation is tenant-scoped, defaults to dry-run, reports bounded counts without storage keys or cross-tenant storage totals, and can resolve only that tenant's stale deletes. The scheduled operator job uses the complete metadata inventory and can remove proven orphan blobs only when its inventory is not truncated. Uploads send raw bytes as `application/octet-stream`, carry the percent-encoded UTF-8 display filename in `x-forgetbase-attachment-filename-encoded`, and carry the allowlisted media type in `x-forgetbase-attachment-media-type`. Reader and admin clients receive metadata without storage keys.
...
Current implementation note: Phase 5 exposes `/exports/ai-package` plus SDK, CLI, and MCP wrappers. The default package is JSON, permission-filtered, citation-bearing, and optimized for agent connectors rather than human page rendering. The same route also supports `format=okf&okfVersion=0.1`, generating a versioned Open Knowledge Format projection with deterministic Markdown files, root `okf_version`, source asset version metadata, source content hashes, and a projection hash. OKF is enabled by default as an export format, but ForgetBase asset versions remain canonical and export permission filtering remains mandatory before OKF generation.
...
Current implementation note: asset updates, review completion, and publishing each create a new immutable governed version containing the full asset snapshot plus its active instruction/document content. Active content remains scoped to `current_version_id`. Authenticated users can fetch a version snapshot to inspect historical content without changing the current pointer. Maintainers/admins can list a review queue of assets that are stale, not approved, not active, or explicitly included for governance sweeps. Review snapshots capture status, next review date, and optional source reference; publish snapshots capture the active/approved transition and optional review date. Both operations record audit events through the API and reindex retrieval chunks. Public anonymous read/search/export only exposes `public-demo` assets after the published state is true. Restoring a version moves the current pointer back to an existing immutable snapshot, records an audit event through the API, and reindexes retrieval chunks for that asset.
...
- `forgetbase login`
- `forgetbase auth logout`
- `forgetbase auth sessions`
- `forgetbase auth session-revoke`
- `forgetbase auth oidc-start`
- `forgetbase auth oidc-callback`
- `forgetbase health`
- `forgetbase validate`
- `forgetbase import`
- `forgetbase search`
- `forgetbase agent query`
- `forgetbase agent feedback`
- `forgetbase agent feedback-list`
- `forgetbase agent eval`
- `forgetbase agent eval-runs`
- `forgetbase agent eval-summary`
- `forgetbase agent action-execute`
- `forgetbase agent action-list`
- `forgetbase agent action-decision`
- `forgetbase admin model-providers`
- `forgetbase admin model-provider-health`
- `forgetbase admin model-provider-set`
- `forgetbase admin auth-providers`
- `forgetbase admin auth-provider-set`
- `forgetbase admin secret-reference-policy`
- `forgetbase admin secret-reference-policy-set`
- `forgetbase admin action-execution-policy`
- `forgetbase admin action-execution-policy-set`

Current action policy CLI note: `forgetbase admin action-execution-policy-set` accepts `--max-requests-per-hour` to tune the tenant-scoped request cap before durable action requests are stored and `--approval-expires-in-minutes` to bound how long pending approvals can execute. `forgetbase agent action-execute` accepts `--idempotency-key` so agent harness retries can return the original request instead of creating duplicates or consuming the hourly cap again.
- `forgetbase admin pii-redaction-policy`
- `forgetbase admin pii-redaction-policy-set`
- `forgetbase admin managed-query-cache`
- `forgetbase admin managed-query-cache-policy`
- `forgetbase admin managed-query-cache-policy-set`
- `forgetbase admin managed-query-cache-delete`
- `forgetbase admin managed-query-cache-purge`
- `forgetbase admin managed-query-retention-policy`
- `forgetbase admin managed-query-retention-policy-set`
- `forgetbase assets review-queue`
- `forgetbase assets review`
- `forgetbase fetch`
- `forgetbase export`
- `forgetbase users`
- `forgetbase keys`
- `forgetbase telemetry`
- `forgetbase telemetry retention`
- `forgetbase telemetry retention-set`
- `forgetbase telemetry purge`
