...
Status: not implemented as a hosted product. The repo has a sanitized Railway private-alpha template and reusable container services, but tenant provisioning, billing, managed backups, hosted retention defaults, support posture, and hosted-only feature boundaries remain future work that requires explicit product and operations decisions.
