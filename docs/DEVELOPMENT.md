...
With Docker Compose API running and `AGENTIC_CMS_OIDC_STATE_SECRET` set for the API service, run the local fake-provider OIDC smoke check:
...
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- health --api-url http://127.0.0.1:3100
```
...
```bash
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- validate --file corpus/demo/assets.json --as-of 2026-06-16
```
...
```bash
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- assets update <stable-id> --file update.json
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- assets version <stable-id> --version-number 1
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- assets review-queue --as-of 2026-06-16
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- assets review <stable-id> --review-due-at 2027-06-30 --change-note "Review current guidance"
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- assets publish <stable-id> --review-due-at 2027-06-30 --change-note "Approve release"
...
```bash
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- auth me
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- auth login --email admin@example.test --password "$AGENTIC_CMS_PASSWORD" --device-label "CLI on work laptop"
AGENTIC_CMS_API_KEY="$AGENTIC_CMS_LOGIN_API_KEY" npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- auth sessions
AGENTIC_CMS_API_KEY="$AGENTIC_CMS_LOGIN_API_KEY" npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- auth session-revoke --session-id <login-session-id>
AGENTIC_CMS_API_KEY="$AGENTIC_CMS_LOGIN_API_KEY" npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- auth logout
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- auth user-create --email reader@example.test --display-name "Reader" --role reader
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- auth user-list
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- auth user-update --user-id <reader-user-id> --display-name "Reader" --status active
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- auth service-account-create --slug automation --name "Automation" --role reader
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- auth service-account-list
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- auth service-account-update --service-account-id <service-account-id> --status active
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- admin service-account-policy
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- admin service-account-policy-set --max-service-accounts 50 --max-active-api-keys 5 --default-key-expires-in-days 90
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- auth group-create --slug ai-readers --name "AI Readers"
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- auth group-member-add --group-id <group-id> --user-id <reader-user-id>
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- auth group-members --group-id <group-id>
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- auth group-member-remove --group-id <group-id> --user-id <reader-user-id>
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- auth group-delete --group-id <group-id>
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- auth api-key-create --user-id <reader-user-id> --name reader --scopes asset:read
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- auth api-key-create --service-account-id <service-account-id> --name automation --scopes agent:execute
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- auth api-key-list
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- auth api-key-rotation-due --due-within-days 30
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- auth api-key-rotate --api-key-id <old-api-key-id> --name reader-rotated
...
```bash
TEST_DATABASE_URL=postgres://agentic_cms:agentic_cms_dev@127.0.0.1:5432/agentic_cms npx -y pnpm@11.7.0 test
```
...
DATABASE_URL=postgres://agentic_cms:agentic_cms_dev@127.0.0.1:5432/agentic_cms npx -y pnpm@11.7.0 --filter @agentic-cms/worker start -- --retention-once
...
DATABASE_URL=postgres://agentic_cms:agentic_cms_dev@127.0.0.1:5432/agentic_cms npx -y pnpm@11.7.0 --filter @agentic-cms/worker start -- --api-key-rotation-reminders-once --due-within-days 14 --dedupe-window-hours 24
DATABASE_URL=postgres://agentic_cms:agentic_cms_dev@127.0.0.1:5432/agentic_cms npx -y pnpm@11.7.0 --filter @agentic-cms/worker start -- --managed-query-evals-once
...
```bash
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- agent query --query "PII redaction" --mode provider-routed --provider openai --cache true
```
...
```bash
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- agent feedback --telemetry-event-id retrieval_1 --query "PII redaction" --outcome accepted --factual-citation-accuracy 5
...
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- admin retrieval-ranking-policy
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- admin retrieval-ranking-policy-set --agent-instruction-weight 1.2 --asset-summary-weight 1.1 --human-document-weight 1 --exact-phrase-boost 0.25
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- admin managed-query-policy
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- admin managed-query-policy-set --default-mode deterministic-retrieval --allowed-modes deterministic-retrieval,provider-routed --minimum-citation-count 1 --require-grounded false
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- admin managed-query-cache
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- admin managed-query-cache-policy
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- admin managed-query-cache-policy-set --cache-enabled true --max-cache-ttl-seconds 3600
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- admin managed-query-cache-delete --cache-key <cache-key>
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- admin managed-query-cache-purge
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- admin managed-query-cache-purge --execute
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- admin managed-query-retention-policy
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- admin managed-query-retention-policy-set --prompt-capture-mode metadata-only --response-capture-mode metadata-only --metadata-retention-days 30
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- admin action-execution-policy
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- admin action-execution-policy-set --enabled true --allowed-action-types create-task-record --require-approval true --dry-run-default false --kill-switch false --max-requests-per-hour 60 --approval-expires-in-minutes 1440
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- agent action-execute --action-type create-task-record --title "Create review task" --idempotency-key review-task-2026-06-17 --dry-run false # requires admin or agent:execute
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- agent action-list
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- agent action-decision --action-request-id <action-request-id> --decision approve --reason "Approved for internal task tracking"
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- admin secret-reference-policy
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- admin secret-reference-policy-set --allowed-prefixes AGENTIC_CMS_,OPENAI_,ANTHROPIC_,OPENROUTER_,ENTRA_,OIDC_ --allowed-env-vars CUSTOM_PROVIDER_SECRET --allow-unlisted false
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- admin pii-redaction-policy
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- admin pii-redaction-policy-set --redaction-enabled true --enabled-rule-kinds api-key,bearer-token,credit-card,email,government-id,ip-address,jwt,phone,url-secret
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- telemetry retention
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- telemetry retention-set --retrieval-event-days 30 --audit-event-days 365 --feedback-days 90
...
```bash
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- agent eval --file corpus/demo/evals.json --minimum-pass-rate 1 --tag-minimum-pass-rates citation-accuracy=1,policy-compliance=1,task-completion-quality=1 --fail-on-threshold true
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- agent eval-runs --limit 10
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- agent eval-summary --limit 20
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- admin managed-query-eval-schedule-policy
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- admin managed-query-eval-schedule-policy-set --enabled true --interval-minutes 1440 --file corpus/demo/evals.json
```
...
```bash
export AGENTIC_CMS_OIDC_STATE_SECRET="$(openssl rand -base64 32)"
```

Start and complete an OIDC login from the CLI:

```bash
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- auth oidc-start --provider microsoft-entra --tenant-id tenant_demo --redirect-uri http://localhost:5175/
npx -y pnpm@11.7.0 --filter @agentic-cms/cli start -- auth oidc-callback --provider microsoft-entra --tenant-id tenant_demo --code <authorization-code> --state <signed-state> --nonce <nonce> --code-verifier <code-verifier>
```
...
node apps/worker/dist/index.js --api-key-rotation-reminders-once --due-within-days 14 --dedupe-window-hours 24 --notification-webhook-url https://ops.example.test/agentic-cms/key-rotation --execute
...
Retention maintenance uses the stored tenant policy and defaults to dry-run. Add `--execute` for one-off deletion after reviewing counts. For the long-running worker, scheduled retention is disabled by default; set `AGENTIC_CMS_RETENTION_PURGE_ENABLED=true` to schedule it, keep `AGENTIC_CMS_RETENTION_PURGE_DRY_RUN=true` for previews, and set `AGENTIC_CMS_RETENTION_PURGE_INTERVAL_MS` to control the interval.

Managed-query cache cleanup removes expired cache rows only and defaults to dry-run. Add `--execute` to `--cache-purge-once` after reviewing counts. For the long-running worker, scheduled cache cleanup is disabled by default; set `AGENTIC_CMS_CACHE_PURGE_ENABLED=true` to schedule it, keep `AGENTIC_CMS_CACHE_PURGE_DRY_RUN=true` for previews, and set `AGENTIC_CMS_CACHE_PURGE_INTERVAL_MS` to control the interval.

API-key rotation reminder maintenance reports service-account keys that are expired, close to expiry, or missing expiry metadata. It defaults to dry-run and exposes tenant counts, reminder counts, audit-event counts, duplicate-skip counts, and notification-delivery counts in the worker log. Add `--execute` to `--api-key-rotation-reminders-once` after reviewing counts; execution records one audit event per tenant with key IDs, owner type, state, and days until expiry, but no raw secrets or secret previews. The default `--dedupe-window-hours 24` skips matching tenant/key-state evidence already audited inside the window; set it to `0` only for deliberate duplicate evidence. Set `AGENTIC_CMS_API_KEY_ROTATION_REMINDERS_WEBHOOK_URL` or pass `--notification-webhook-url` to deliver executed reminders to a webhook after audit evidence is written; dry-runs and duplicate-skipped reminders do not call the webhook. Put the optional HMAC secret in `AGENTIC_CMS_API_KEY_ROTATION_REMINDERS_WEBHOOK_SIGNING_SECRET`, not on the command line, and use `AGENTIC_CMS_API_KEY_ROTATION_REMINDERS_WEBHOOK_TIMEOUT_MS` or `--notification-webhook-timeout-ms` to adjust the default 5000ms timeout. For the long-running worker, scheduled reminders are disabled by default; set `AGENTIC_CMS_API_KEY_ROTATION_REMINDERS_ENABLED=true` to schedule them, keep `AGENTIC_CMS_API_KEY_ROTATION_REMINDERS_DRY_RUN=true` for previews, set `AGENTIC_CMS_API_KEY_ROTATION_REMINDERS_DUE_WITHIN_DAYS`, set `AGENTIC_CMS_API_KEY_ROTATION_REMINDERS_DEDUPE_WINDOW_HOURS`, and set `AGENTIC_CMS_API_KEY_ROTATION_REMINDERS_INTERVAL_MS`.

Managed-query eval schedule maintenance runs due tenant schedule policies and defaults to dry-run. Add `--execute` after reviewing due counts. Execution searches the tenant corpus directly as system maintenance, records scheduled retrieval telemetry, writes redacted eval-run history, updates the schedule policy last-run fields, and records `agent.eval.scheduled_run` audit evidence without raw query text. For the long-running worker, scheduled eval execution is disabled by default; set `AGENTIC_CMS_MANAGED_QUERY_EVALS_ENABLED=true` to schedule it, keep `AGENTIC_CMS_MANAGED_QUERY_EVALS_DRY_RUN=true` for previews, set `AGENTIC_CMS_MANAGED_QUERY_EVALS_LIMIT`, and set `AGENTIC_CMS_MANAGED_QUERY_EVALS_INTERVAL_MS`.

Action approval expiry maintenance finds `approval-required` action requests whose `approvalExpiresAt` is in the past and defaults to dry-run. Add `--execute` after reviewing candidate counts. Execution marks each stale request `expired`, records `agent.action.approval_expiry` audit evidence without executing the action, and leaves non-expired pending requests untouched. For the long-running worker, scheduled action approval expiry is disabled by default; set `AGENTIC_CMS_ACTION_APPROVAL_EXPIRY_ENABLED=true` to schedule it, keep `AGENTIC_CMS_ACTION_APPROVAL_EXPIRY_DRY_RUN=true` for previews, set `AGENTIC_CMS_ACTION_APPROVAL_EXPIRY_LIMIT`, and set `AGENTIC_CMS_ACTION_APPROVAL_EXPIRY_INTERVAL_MS`.
...
const client = new Client({ name: 'agentic-cms-smoke', version: '0.1.0' });
