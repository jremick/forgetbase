...
export class PostgresRegistryRepository implements RegistryRepository {
  ...
  async createAsset(input: AssetCreateInput): Promise<AssetDetail> {
    ...
    try {
      ...
      const contentHash = hashAssetContent(parsed);
      ...
      if (parsed.instruction) {
        await client.query(
          ...
          [
            assetId,
            versionId,
            parsed.instruction.instructionKind,
            parsed.instruction.targetAgents,
            parsed.instruction.body,
            JSON.stringify(parsed.instruction.inputContract),
            JSON.stringify(parsed.instruction.outputContract),
            parsed.instruction.constraints,
            parsed.instruction.examples,
            parsed.instruction.failureModes,
            parsed.instruction.escalation ?? null
          ]
        );
      }
      ...
      if (parsed.humanDocument) {
        await client.query(
          ...
          [
            assetId,
            versionId,
            parsed.humanDocument.format,
            parsed.humanDocument.body,
            JSON.stringify(parsed.humanDocument.renderOptions),
            parsed.humanDocument.linkedInstructionIds
          ]
        );
      }
    ...
    try {
      ...
      const version = await client.query<AssetVersionRow>(
        ...
        [
          asset.id,
          versionNumber,
          hashAssetContent({ stableId, update: parsed }),
          JSON.stringify(parsed.metadata),
          asset.owner_id,
  ...
  async restoreAssetVersion(stableId: string, input: AssetRestoreInput): Promise<AssetDetail | null> {
    ...
    try {
      await client.query("BEGIN");
      const target = await client.query<{ asset_id: string; version_id: string }>(
        `
          SELECT assets.id AS asset_id, asset_versions.id AS version_id
          FROM assets
      ...
      await client.query(
        "UPDATE assets SET current_version_id = $1, updated_at = now() WHERE id = $2",
        [row.version_id, row.asset_id]
      );
    ...
    try {
      ...
      const result = await client.query<AssetRow>(
        `
          ...
          RETURNING *
        `,
        [parsed.tenantId, stableId, parsed.status, parsed.reviewDueAt, parsed.sourceRef ?? null]
      );
      ...
      const result = await client.query<AssetRow>(
        `
          ...
          SET
            ...
            status = 'approved',
            review_due_at = COALESCE($3::date, review_due_at),
            updated_at = now()
          WHERE tenant_id = $1
            AND stable_id = $2
          RETURNING *
        `,
        [parsed.tenantId, stableId, parsed.reviewDueAt ?? null]
      );
      const asset = result.rows[0];
...
export class InMemoryRegistryRepository implements RegistryRepository {
  ...
  async createAsset(input: AssetCreateInput): Promise<AssetDetail> {
    ...
    const version = assetVersionSchema.parse({
      ...
      assetId,
      versionNumber: 1,
      contentHash: hashAssetContent(parsed),
  ...
  async updateAsset(stableId: string, input: AssetUpdateInput): Promise<AssetDetail | null> {
    ...
    const version = assetVersionSchema.parse({
      ...
      assetId: asset.id,
      versionNumber,
      contentHash: hashAssetContent({ stableId, update: parsed }),
      metadata: parsed.metadata,
  ...
  async reviewAsset(stableId: string, input: AssetReviewInput): Promise<AssetDetail | null> {
    ...
    const reviewed = assetDetailSchema.parse({
      ...detail,
      asset: {
        ...detail.asset,
        status: parsed.status,
  ...
  async publishAsset(stableId: string, input: AssetPublishInput): Promise<AssetDetail | null> {
    ...
    const published = assetDetailSchema.parse({
      ...detail,
      asset: {
        ...detail.asset,
...
async function getAssetVersionSnapshot(
  ...
  return assetVersionSnapshotSchema.parse({
    asset: mapAssetRow(requireRow(asset)),
    version: mapAssetVersionRow(requireRow(version)),
    instructionObjects: instructions.rows.map(mapInstructionObjectRow),
    humanDocuments: humanDocuments.rows.map(mapHumanDocumentRow)
  });
}
...
async function insertInstruction(
  ...
  assetId: string,
  versionId: string,
  instruction: NonNullable<AssetUpdateInput["instruction"]>
): Promise<void> {
  await client.query(
    `
      INSERT INTO instruction_objects (
...
async function insertHumanDocument(
  ...
  assetId: string,
  versionId: string,
  document: NonNullable<AssetUpdateInput["humanDocument"]>
): Promise<void> {
  await client.query(
    `
      INSERT INTO human_documents (
...
function hashAssetContent(input: unknown): string {
