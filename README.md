...
ForgetBase is an open-core, agent-native instruction management platform for AI teams and power users.
...
This repository is a private beta candidate for the self-hosted core. The frozen private-beta machine-consumer lane is documented in [ForgetBase Private Beta Contract](docs/BETA_PRIVATE_CONTRACT.md); broader admin, hosted, provider, and identity surfaces remain preview.

Expected private beta instability:

- API routes, CLI flags, MCP tool names, and package boundaries outside the private-beta contract may change.
...
Most knowledge hubs are built for humans first and then adapted for AI retrieval. ForgetBase reverses that priority.
...
- Postgres retrieval chunks with citation metadata, transparent lexical/vector/hybrid ranking metadata, admin-configurable tenant ranking weights, deterministic hash embeddings or opt-in OpenAI-compatible provider embeddings stored in `pgvector`, and provider/model/dimension metadata to avoid mixed vector spaces
...
- GitHub Actions CI for typechecking, build, strict demo corpus validation, static Compose config parsing, OpenAPI drift, claims lint, focused beta contracts, and Postgres-backed tests
